...
import br.com.clinicaintegrada.financeiro.dao.HistoricoDao;
...
import br.com.clinicaintegrada.impressao.DemonstrativoAcordo;
import br.com.clinicaintegrada.impressao.Promissoria;
    ...
    public List<Movimento> atualizaContaCobrancaMovimento(List<Movimento> lista) {
        ...
        for (int i = 0; i < lista.size(); i++) {
            ...
            if (scc.getTipoServico().getId() == 4) {
                ...
                Historico his_pesquisa = historicoDao.findHistoricoByContaCobrancaAndMovimento(bol.getContaCobranca().getId(), lista.get(i).getId());
...
////                    String entidade = filialDao.pesquisaRegistroPorCliente(SessaoCliente.get().getId());
...
//                        Messages.error("Erro", mensagemErroMovimento);
//                        //continue;
//                    } else {
...
//            }
    ...
    public byte[] imprimirAcordo(List<Movimento> lista, Acordo acordo, Historico historico, String emailContato, boolean imprimir_pro) {
        try {
            ...
            if (!lista.isEmpty()) {
                Dao dao = new Dao();
                filial = (Filial) dao.find(new Filial(), 1);
            ...
            try {
                pe = pessoaEnderecoDao.pesquisaPessoaEnderecoPorPessoaTipo(pessoa.getId(), 2);
                swap[2] = pe.getEndereco().getEnderecoSimplesToString();
                swap[3] = pe.getNumero();
                swap[4] = pe.getComplemento();
                swap[5] = pe.getEndereco().getBairro().getDescricao();
            ...
            try {
                pe = pessoaEnderecoDao.pesquisaPessoaEnderecoPorPessoaTipo(filial.getId(), 3);
            ...
            try {
                pe = pessoaEnderecoDao.pesquisaPessoaEnderecoPorPessoaTipo(juridica.getContabilidade().getPessoa().getId(), 3);
...
//            Boleto boleto = boletoDao.findBoletosByNrCtrBoleto(lista.get(i).getNrCtrBoleto());
    ...
    public byte[] imprimirPromissoria(List<Movimento> lista, boolean imprimirVerso) {
        try {
            ...
            try {
                juridica = juridicaDao.pesquisaJuridicaPorPessoa(pessoa.getId());
                swap[0] = juridica.getPessoa().getNome();
                swap[1] = juridica.getPessoa().getDocumento();
            } catch (Exception e) {
                swap[0] = "";
            ...
            try {
                PessoaEndereco pe = pessoaEnderecoDao.pesquisaPessoaEnderecoPorPessoaTipo(pessoa.getId(), 2);
                swap[2] = pe.getEndereco().getEnderecoSimplesToString();
                swap[3] = pe.getNumero();
                swap[4] = pe.getComplemento();
                swap[5] = pe.getEndereco().getBairro().getDescricao();
            ...
            try {
                PessoaEndereco pe = pessoaEnderecoDao.pesquisaPessoaEnderecoPorPessoaTipo(filial.getId(), 3);
                swap[9] = filial.getMatriz().getPessoa().getNome();
                swap[10] = filial.getMatriz().getPessoa().getTipoDocumento().getDescricao();
                swap[11] = pe.getEndereco().getDescricaoEndereco().getDescricao();
                swap[12] = pe.getEndereco().getLogradouro().getDescricao();
            ...
            try {
                PessoaEndereco pe = pessoaEnderecoDao.pesquisaPessoaEnderecoPorPessoaTipo(juridica.getContabilidade().getPessoa().getId(), 3);
                swap[34] = juridica.getContabilidade().getPessoa().getNome();
                swap[24] = pe.getEndereco().getDescricaoEndereco().getDescricao();
                swap[25] = pe.getEndereco().getLogradouro().getDescricao();
                swap[26] = pe.getNumero();
            ...
            while (i < lista.size()) {
                ...
                vetor.add(new Promissoria(
                        ...
                        swap[16], // sinCidade
                        swap[17], // sinUF
                        lista.get(i).getVencimentoString(),//DataHoje.DataToArray(lista.get(i).getVencimento())[2]+"-"+DataHoje.DataToArray(lista.get(i).getVencimento())[1]+"-"+DataHoje.DataToArray(lista.get(i).getVencimento())[0], // vencto
    ...
    public byte[] imprimirPlanilha(List<Movimento> lista, List<Float> listaValores, boolean calculo, boolean imprimirVerso) {
        try {
            ...
            while (i < lista.size()) {
                ...
                if (calculo) {
                    lAcres = movimentoDao.findAcrescimoByMovimento(lista.get(i).getId());
                    if (lAcres.isEmpty()) {
                ...
                vetor.add(new DemonstrativoAcordo(
                        ...
                        //acordo.getContato(), // contato
                        0, // codacordo
                        lista.get(i).getVencimentoString(), // data
                        "", // contato
                        swap[0], // razao
                        swap[1], // cnpj
                        swap[2], //endereco
    ...
    public byte[] imprimirAcordoPromissoria(List<Movimento> lista, Acordo acordo, Historico historico, String emailContato, boolean imprimir_pro) {
        try {
            ...
            MovimentoDao movimentoDao = new MovimentoDao();
            int qnt = movimentoDao.findMovimentoByAcordoAberto(acordo.getId()).size();
            while (i < lista.size()) {
                List<List> lAcres = new ArrayList<>();
