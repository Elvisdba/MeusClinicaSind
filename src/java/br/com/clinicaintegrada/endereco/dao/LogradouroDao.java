...
public class LogradouroDao extends DB {
    ...
    public Logradouro pesquisaLogradouroPorDescricao(String descricao) {
        ...
        try {
            Query query = getEntityManager().createNativeQuery("SELECT L.* FROM end_logradouro AS L WHERE UPPER(FUNC_TRANSLATE(L.ds_descricao)) = '" + AnaliseString.removerAcentos(descricao) + "'", Logradouro.class);
