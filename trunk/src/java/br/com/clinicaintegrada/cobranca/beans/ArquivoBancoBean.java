package br.com.clinicaintegrada.cobranca.beans;
...
import br.com.clinicaintegrada.cobranca.CaixaFederalSicob;
import br.com.clinicaintegrada.cobranca.Cobranca;
import br.com.clinicaintegrada.cobranca.FEBRABAN.RegistroDetalheSegmentoP;
import br.com.clinicaintegrada.cobranca.FEBRABAN.RegistroDetalheSegmentoQ;
import br.com.clinicaintegrada.cobranca.FEBRABAN.RegistroHeaderLote;
import br.com.clinicaintegrada.cobranca.FEBRABAN.RegistroTrailerLote;
...
public class ArquivoBancoBean implements Serializable {
    ...
    private int indexArquivos;
    private FileOutputStream file;
    private FileWriter writer;
    private BufferedWriter buffWriter;
    private Juridica juridica;
    private Object retornoBanco;
    private PessoaEndereco pessoaEndereco;
    private Movimento movimento;
    private ContaCobranca contaCobranca;
    private String conteudoRemessa;
    private String jur;
    ...
    private String dtLiquidacao;
    private String nossoNum;
    private String vlrLiquido;
    private String vlrTaxa;
    private String vlrPago;
    private String msgOk;
    private String lblPendente;
    private List listaJuridica;
    private List listaMovimentos;
    private List listaArquivoRemessa;
    private List listaArquivoRetorno;
    private List listaRetorno;
    private List<DataObject> listaDocumentos;
    private List listaPasta;
    private boolean outros;
    private boolean carregaPastas;
    private String strServicos;

    private int index_contribuicao;
    ...
    public ArquivoBancoBean() {
        ...
        retornoBanco = new Object();
        pessoaEndereco = new PessoaEndereco();
        movimento = new Movimento();
        contaCobranca = new ContaCobranca();
        conteudoRemessa = null;
        jur = null;
        pesEnd = null;
        rua = null;
        ...
        msgOk = "";
        lblPendente = "";
        listaJuridica = new Vector();
        listaMovimentos = new Vector();
        listaArquivoRemessa = new Vector();
        listaArquivoRetorno = new Vector();
...
//        }
//        
//        Dirs.criar("Arquivos/retorno/" + cod + "");
//        Dirs.criar("Arquivos/retorno/" + cod + "/pendentes");
//
...
//                Messages.error("Erro "+ fl.getName(), "Arquivo não pode ser enviado, verifique se a CONTRIBUIÇÃO e a CONTA estão corretas!");
//                fl.delete();
//            }
//            
...
//                linha_file.delete();
//            }
//            Messages.info("OK", "Arquivos Excluídos!");
//            loadListaArquivosBaixar();
//        } catch (Exception e) {
//            Messages.error("Erro", "Não foi possível excluir arquivos, tente novamente!");
//        }
//    }
//
...
//
    public String download() {
        ...
        try {
            createFileTxt(movimentoDao.findMovimentosByLote(21));
        } catch (Exception e) {
    ...
    public boolean createFileTxt(List<Movimento> movs) {
        if (movs.isEmpty()) {
            return false;
        }
        PessoaEnderecoDao pessoaEnderecoDao = new PessoaEnderecoDao();
        MensagemCobrancaDao mensagemCobrancaDao = new MensagemCobrancaDao();
        BoletoDao boletoDao = new BoletoDao();
        RemessaBancoDao remessaBancoDao = new RemessaBancoDao();
        Boleto boleto = new Boleto();
        try {
            Dirs.create("downloads/remessa");
            ...
            Movimento m = movs.get(0);
            boleto = boletoDao.findBoletosByNrCtrBoleto(m.getNrCtrBoleto());

            // HEADER LOTE
            RegistroHeaderLote registroHeaderLote = new RegistroHeaderLote();
            registroHeaderLote.setCodigoBancoCompensacao(contaCobranca.getContaBanco().getBanco().getNumero());
            registroHeaderLote.setLoteServico("0001");
            // Analisar
            registroHeaderLote.setCodigoBancoCompensacao("001");
            registroHeaderLote.setTipoOperacao("R");
            registroHeaderLote.setTipoInscricao("" + m.getLote().getFilial().getFilial().getPessoa().getTipoDocumento().getId());
            if (m.getLote().getFilial().getFilial().getPessoa().getTipoDocumento().getId() == 1) {
                registroHeaderLote.setNumeroInscricao("000" + AnaliseString.limpaDocumento(m.getLote().getFilial().getFilial().getPessoa().getDocumento()));
            } else if (m.getLote().getFilial().getFilial().getPessoa().getTipoDocumento().getId() == 2) {
                registroHeaderLote.setNumeroInscricao("0" + AnaliseString.limpaDocumento(m.getLote().getFilial().getFilial().getPessoa().getDocumento()));
            }
            registroHeaderLote.setNome(AnaliseString.removerAcentos(m.getLote().getFilial().getFilial().getPessoa().getNome()));
            //registroHeaderLote.setConvenio("");
            registroHeaderLote.setAgencia(contaCobranca.getContaBanco().getAgencia());
            registroHeaderLote.setAgenciaDV(calculaDigitoAgencia(m, boleto));
            registroHeaderLote.setConta(contaCobranca.getContaBanco().getConta());
            //registroHeaderLote.setContaDigitoVerificador("");
            registroHeaderLote.setDataCredito(AnaliseString.limpaDocumento(m.getLote().getEmissaoString()));
            registroHeaderLote.write();
            buffWriter.write(RegistroHeaderLote.LINHA);
            buffWriter.newLine();

            // REGISTROS DETALHES
            RegistroDetalheSegmentoP segmentoP = new RegistroDetalheSegmentoP();
            RegistroDetalheSegmentoQ segmentoQ = new RegistroDetalheSegmentoQ();
            // SEGMENTO P
            for (int i = 0; i < movs.size(); i++) {
                segmentoP.setCodigoBancoCompensacao(contaCobranca.getContaBanco().getBanco().getNumero());
                segmentoP.setLoteServico("0001");
                segmentoP.setTipoRegistro("3");
                segmentoP.setSequencialLote("" + i + 1);
                segmentoP.write();
                buffWriter.write(RegistroDetalheSegmentoP.LINHA);
                buffWriter.newLine();

                // SEGMENTO Q
                segmentoQ.setCodigoBancoCompensacao(contaCobranca.getContaBanco().getBanco().getNumero());
                segmentoQ.setLoteServico("0001");
                segmentoQ.setTipoRegistro("3");
                segmentoQ.setSequencialLote("" + i + 1);
                segmentoQ.write();
                buffWriter.write(RegistroDetalheSegmentoQ.LINHA);
                buffWriter.newLine();

                segmentoP = new RegistroDetalheSegmentoP();
                segmentoQ = new RegistroDetalheSegmentoQ();
            }

            // TRAILER LOTE
            RegistroTrailerLote registroTrailerLote = new RegistroTrailerLote();
            registroTrailerLote.setCodigoBancoCompensacao(contaCobranca.getContaBanco().getBanco().getNumero());
            registroTrailerLote.setLoteServico("0001");
            registroTrailerLote.setTipoRegistro("5");
            registroTrailerLote.setQuantidadeRegistrosLote("" + movs.size());
            buffWriter.write(RegistroTrailerLote.LINHA);
            buffWriter.newLine();
...
//            String docs = AnaliseString.limpaDocumento(movs.get(0).getLote().getFilial().getFilial().getPessoa().getDocumento());
...
//
//                // Data Vencimento
//                conteudoRemessa += movs.get(i).getVencimentoString().replace("/", "");
//
...
//
//                pessoaEndereco = pessoaEnderecoDao.pesquisaPessoaEnderecoPorPessoaTipo(movs.get(i).getPessoa().getId(), 3);
//
...
//
//                conteudoRemessa += "          ";
//                MensagemCobranca mensa = mensagemCobrancaDao.findByMovimento(movs.get(i).getId());
//                if (mensa != null) {
//                    // 1 - Mensagem Boleto
//                    conteudoRemessa += (AnaliseString.removerAcentos(mensa.getMensagemBoleto().getMensagem().toUpperCase()) + "                                        ").substring(0, 40);
//
//                    // 2 - Mensagem Boleto
//                    conteudoRemessa += (AnaliseString.removerAcentos(mensa.getMensagemBoleto().getMensagemCompensacao().toUpperCase()) + "                                        ").substring(0, 40);
//                } else {
//                    conteudoRemessa += "                                                                                ";
//                }
//
...
//                buffWriter.write(conteudoRemessa);
//                buffWriter.newLine();
//                boleto = boletoDao.findBoletosByNrCtrBoleto(movs.get(i).getNrCtrBoleto());
//                Object obj = remessaBancoDao.findByContaCobranca(boleto.getContaCobranca().getId());
//                rb = new RemessaBanco();
//                if (obj != null) {
...
//        }
//    }
    public String calculaDigitoAgencia(Movimento m, Boleto b) {
        switch (b.getContaCobranca().getContaBanco().getBanco().getId()) {
            case ArquivoRetorno.BANCO_BRASIL:
                Cobranca c = new BancoDoBrasil(m, b);
                return c.moduloOnze(b.getContaCobranca().getContaBanco().getAgencia());
        }
        return "";
    }
...
//            Messages.warn("Atenção", "Não existe nenhum Documento para ser Atualizado!");
//            return;
//        }
//        MovimentoDB db = new MovimentoDBToplink();
...
//                            sv.desfazerTransacao();
//                            Messages.error("Erro", "Não foi possível atualizar Lista!");
//                            return;
//                        }
//
...
//                    sv.comitarTransacao();
//
//                    Messages.info("OK", "Documento "+listaDocumento.getArgumento1()+" atualizado!");
//                }
...
//            } 
//            
//            Messages.info("Sucesso", "Arquivos Baixados");
//            loadListaArquivosBaixar();
//            loadListaDocumentos();
//        } catch (Exception e) {
//            
...
//        if (listaDocumentos.isEmpty()){
//            Messages.warn("Atenção", "Não existe nenhum Documento para ser Impresso!");
//            return;
//        }
//        try {
//            FacesContext faces = FacesContext.getCurrentInstance();
...
//                Messages.error("Erro", erro.getMessage());
//            }
