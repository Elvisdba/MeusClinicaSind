...
public class Dao extends DB implements DaoInterface {
    ...
    public List findByDescription(String className, String description) {
        return findByDescription(className, description, "");
    }
    ...
    public boolean existDescriptionInField(String className, String field, String description) {
        try {
            Query qry = getEntityManager().createQuery("SELECT OB FROM " + className + " OB  WHERE UPPER(OB." + field + ") = :description");
